...
case $kernrel in
  ...
  *)
    echo "WARNING: Using wrong kernel release. Use GhostBSD 20.04 or later to build iso."
    ;;
esac
...
packages_software()
{
  ...
  cat ${cwd}/packages/${desktop} | sed '/^#/d' | sed '/\!i386/d' | xargs pkg -c ${release} install -y
...
image()
{
  ...
  which transmission-create && ( transmission-create -o ${iso}/${torrent} -t ${tracker1} -t ${tracker3} -t ${tracker3} ${isopath} ; chmod 644 ${iso}/${torrent} ) || true
  cd -
}
